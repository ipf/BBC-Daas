...
/**
 * A simple In Memory user service in Scala
 *
 * @todo The user authentication is only stored in memory - needs to be persisted in mongoDB
 */
class InMemoryUserService(application: Application) extends UserServicePlugin(application) {
	...
	def save(user: Identity): Identity = {
		...
		//val userSave = new User(new ObjectId, user.id.id, user.email.get, user.oAuth2Info.get.accessToken)
